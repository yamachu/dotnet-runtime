...
import { ENVIRONMENT_IS_ESM, ENVIRONMENT_IS_NODE, ENVIRONMENT_IS_PTHREAD, ENVIRONMENT_IS_SHELL, ENVIRONMENT_IS_WEB, INTERNAL, locateFile, Module, MONO, requirePromise, runtimeHelpers } from "./imports";
...
export async function mono_wasm_load_config(configFilePath: string): Promise<void> {
    ...
    try {
        ...
        const configResponse = await runtimeHelpers.fetch(resolvedConfigFilePath);
